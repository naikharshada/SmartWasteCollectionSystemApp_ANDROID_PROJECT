...
import com.example.smartwastecollectionsystem.ui.localuser.ForgotPassActivity;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        backlogin_.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                startActivity(new Intent(ForgotPass1Activity.this, Login1Activity.class));
            }
        });
                ...
                auth.fetchSignInMethodsForEmail(forgotEmail.getEditText().getText().toString()).addOnCompleteListener(new OnCompleteListener<SignInMethodQueryResult>() {
                    ...
                    public void onComplete(@NonNull Task<SignInMethodQueryResult> task) {
                        if(task.getResult().getSignInMethods().isEmpty()){
                            Toast.makeText(ForgotPass1Activity.this, "Enter registered email or create a new account", Toast.LENGTH_SHORT).show();
                            ...
                            auth.sendPasswordResetEmail(forgotEmail.getEditText().getText().toString()).addOnCompleteListener(new OnCompleteListener<Void>() {
                                ...
                                public void onComplete(@NonNull Task<Void> task) {
                                    if(task.isSuccessful()) {
                                        Toast.makeText(ForgotPass1Activity.this, "A reset password email has been sent to your account", Toast.LENGTH_SHORT).show();
                                    } else {
                                        Toast.makeText(ForgotPass1Activity.this, (task.getException()).getMessage(), Toast.LENGTH_SHORT).show();
